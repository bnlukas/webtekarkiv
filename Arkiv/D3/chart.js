...
// venter på dom er indlæst, før den kører koden
document.addEventListener("DOMContentLoaded", () => {
    fetch("albums.json") // henter album data fra albums.json
        .then(response => {
            if (!response.ok) {
...
function createChart(albums) {
    const svg = d3.select("#chart"); //vælger svg 
    const width = 900;
    const height = 700;
    ...
    // tilføjer tooltip som gør at der sker noget når jeg hover over et element 
    d3.select("body")
        .append("div")
        .attr("id", "tooltip")
        .attr("class", "tooltip")
